...
# ---------------------------------------------------------------------------
# Daily smoothie contribution (fixed recipe)
# ---------------------------------------------------------------------------

SMOOTHIE_MICROS: dict[str, float] = {
    "calcium_mg": 659,
    "iron_mg": 3.1,
    "magnesium_mg": 151.6,
    "phosphorus_mg": 696.4,
...
def remaining_targets(sex: Sex, age_group: AgeGroup) -> dict[str, float]:
    """Return max(0, DRI - smoothie) for each micronutrient."""
    dri = DRI_TARGETS[(sex, age_group)]
    return {
        key: max(0.0, dri[key] - SMOOTHIE_MICROS.get(key, 0.0))
        for key in dri
    }
